...
#ifdef DEBUG_DFA
    std::cout << "in state [" << toString(cur_state) << "], input = \'" << input << "\', str = " << cur_str << "\tlookahead=" << (lookahead ? lookahead : ' ') << std::endl;
#endif
...
std::vector<frontend::Token> frontend::Scanner::run() {
    ...
    while (pos < filtered_source.size()) {
...
#ifdef DEBUG_SCANNER
        std::cout << "\nProcessing char: '" << c << "' (0x" << std::hex << (int)c << ")" << std::endl;
#endif
        ...
        while (true) {
            if (dfa.next(c, tk)) {
                tokens.push_back(tk);
#ifdef DEBUG_SCANNER
                std::cout << ">> Accepted token: " << toString(tk.type) << " -> " << tk.value << std::endl;
#endif
    ...
    if (dfa.flush(tk)) {
        tokens.push_back(tk);
#ifdef DEBUG_SCANNER
        std::cout << ">> Flushing final token: " << toString(tk.type) << " -> " << tk.value << std::endl;
#endif
    }
